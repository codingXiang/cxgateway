...
import (
	...
	"github.com/codingXiang/cxgateway/v2/delivery"
	"github.com/codingXiang/cxgateway/v2/middleware"
	"github.com/codingXiang/cxgateway/v2/middleware/track"
...
func NewApiGatewayWithData(configName string, defaultData []byte) delivery.HttpHandler {
	...
	if data, err := configer.Config.GetCore(configName).ReadConfig(defaultData); err == nil {
		...
		gateway.engine.
			...
			Use(track.RequestIDMiddleware(data.GetString("application.appId"))).
			Use(track.RequestVersion(data.GetString("application.version"))).
...
`)
	...
	if data, err := gateway.GetConfig().ReadConfig(gateway.defaultData); err == nil {
		...
		gateway.engine.
			Use(middleware.Logger(), gin.Recovery()).
			Use(track.RequestIDMiddleware(appId)).
			Use(track.RequestVersion(appVersion)).
			Use(middleware.GoI18nMiddleware(data))
